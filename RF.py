...
class ForestTraining(QMainWindow):
    ...
    def _init_ui(self):
        self.setWindowTitle("Random Forest Trainer")
    ...
    def _start_training(self):
        ...
        try:
            self._log(" Applying spectral preprocessing pipeline...")
    ...
    def _load_data(self):
        try:
            ...
            if not path:
                self._log("️ File selection cancelled.")
                return
    ...
    def _plot_results(self, y_pred_test, pi_lower, pi_upper):
        if self.y_test is None or y_pred_test is None:
            self._log("️ Skipping plotting: Missing data.");
            return
        try:
            self.figure.clear();
            self.figure.patch.set_facecolor('#f8f9fa')
    ...
    def _save_model(self):
        ...
        if not path: self._log("️ Model save cancelled."); return
        try:
